...
import { Invoice, InvoiceCompany } from '@/types';
...
export default function AdminInvoiceDetailPage() {
    ...
    const { data, isLoading, isError, error, refetch, isFetching } = useQuery<{ invoice: Invoice; tenant: InvoiceCompany | null }>({
        queryKey: ['invoice', id],
            ...
            <div className="bg-[var(--color-surface)] rounded-xl p-6 border border-[var(--color-border)] shadow-sm">
                {data?.tenant?.sealUrl && (
                    <div className="mb-5 flex justify-end">
                        <img src={data.tenant.sealUrl} alt="Company seal" className="h-20 w-20 object-contain" />
                    </div>
                )}
